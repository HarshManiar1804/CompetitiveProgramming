...
//--------------------------constants----------------------------/
#define INF 0x3f3f3f3f;  // so that there is no ll overflow
...
/*vector<ll> faster_prime(ll n)                        // prime<ll>  (1..n)
{
    ...
    for(ll i=2;i<n;i++)
    {
        ...
        for(ll j=0;j<(ll)prime.size() && prime[j]<=i && i*prime[j]<=n;j++)
        {
