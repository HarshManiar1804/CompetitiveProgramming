...
bool isValid(int nrow,int ncol,int n,int m)
    {
        return (nrow<n && ncol<m && nrow>=0 && ncol>=0);
    }
    void dfs(int row,int col,int drow[],int dcol[],vector<vector<char>>&grid,int n,int m)
    {
        if(grid[row][col]!='1')
            return ;
        if(grid[row][col]=='1')
        ...
        for(int i=0;i<4;i++)
        {
            ...
            {
                dfs(nrow,ncol,drow,dcol,grid,n,m);
            }
        }
    }
