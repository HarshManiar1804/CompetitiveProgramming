...
void solve()
{
    int n, tankLimit, tank = 0, dollar = 0, travelled = 1;
    cin >> n >> tankLimit;
    while (travelled <= n)
    {
        if (tank == 0)
        {
            if ((n - travelled) > tankLimit)
                tank += tankLimit, dollar += travelled * tankLimit;
            else
                tank += (n - travelled), dollar += tank * travelled;
        }
        else if (tank < n - travelled)
        {
            int amountGained = tankLimit - tank;
            tank += amountGained, dollar += travelled * amountGained;
        }
        travelled++;
        tank--; 
    }
