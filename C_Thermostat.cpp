...
void ansGotai()
{
    int left, right, change, startLimit, endLimit;
    cin >> left >> right >> change >> startLimit >> endLimit;
    if (startLimit == endLimit)
    {
        cout << 0;
        return;
    }
    if (!(endLimit >= left || endLimit <= right))
    {
        cout << -1;
        return;
    }
    if (startLimit < endLimit)
    {
        if ((startLimit + change == endLimit) || (startLimit + change < endLimit))
            cout << 1;
        else if ((endLimit + change <= right) || (startLimit - change >= left))
            cout << 2;
        else if ((endLimit - left >= change) && (startLimit + change <= right))
            cout << 3;
        else
            cout << -1;
        return;
    ...
    if ((startLimit - change == endLimit) || (startLimit - change > endLimit))
        cout << 1;
    else if ((endLimit - change >= left) || (startLimit + change <= right))
        cout << 2;
    else if ((right - endLimit >= change) && (startLimit - change >= left))
        cout << 3;
    else
        cout << -1;
}
