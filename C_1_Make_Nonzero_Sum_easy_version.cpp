...
void suAavseJavab()
{
    ...
    vector<pair<int, int>> javab;
    fl(i, 0, num)
    {
        int joSachuCheKeNaiE = i;
        if (vec[joSachuCheKeNaiE] == 0)
            javab.pb({i + 1, i + 1});
        else
        {
            int p = joSachuCheKeNaiE + 1;
            while (p < num && vec[p] == 0)
                p++;
            if (vec[p] == vec[joSachuCheKeNaiE])
            {
                if ((p - joSachuCheKeNaiE) % 2 == 0)
                {
                    javab.pb({joSachuCheKeNaiE + 1, p - 1});
                    javab.pb({p, p + 1});
                }
                else
                    javab.pb({joSachuCheKeNaiE + 1, p + 1});
            ...
            {
                if ((p - joSachuCheKeNaiE) % 2)
                {
                    javab.pb({joSachuCheKeNaiE + 1, p});
                    javab.pb({p + 1, p + 1});
                }
                else
                    javab.pb({joSachuCheKeNaiE + 1, p + 1});
