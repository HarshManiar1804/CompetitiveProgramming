...
//---------------------------------------------------------------/
int n;
int mxtaxi, mxpizza, mxgirl;
struct Node
{
    string N;
    int taxi, pizza, girl;
} a[114];
bool IsTaxi(string x)
{
    set<char> st;
    fl(i, 0, x.size())
    {
...
void ansGotai()
{
    ...
    fl(i, 0, n)
    {
        ...
        a[i].N = s;
        mxtaxi = max(mxtaxi, a[i].taxi);
        mxpizza = max(mxpizza, a[i].pizza);
        mxgirl = max(mxgirl, a[i].girl);
    }
    vector<string> t, p, g;
    fl(i, 0, n) if (a[i].taxi == mxtaxi) t.push_back(a[i].N);
    fl(i, 0, n) if (a[i].pizza == mxpizza) p.push_back(a[i].N);
    fl(i, 0, n) if (a[i].girl == mxgirl) g.push_back(a[i].N);
    cout << "If you want to call a taxi, you should call: ";
    cout << t[0];
    fl(i, 1, t.size()) cout << ", " << t[i];
    cout << "." << endl;
    cout << "If you want to order a pizza, you should call: ";
    cout << p[0];
    fl(i, 1, p.size()) cout << ", " << p[i];
    cout << "." << endl;
    cout << "If you want to go to a cafe with a wonderful girl, you should call: ";
    cout << g[0];
    fl(i, 1, g.size()) cout << ", " << g[i];
    cout << "." << endl;
}
int32_t main()
{
    ll ketlaTestCaseChe = 1;
    // cin >>ketlaTestCaseChe;
    while (ketlaTestCaseChe-- > 0)
    {
        ansGotai();
        // cout << '\n';
    }
