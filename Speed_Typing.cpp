...
void solve()
{
    ...
    fl(i, 0, str2.length())
    {
        if (match.find(str2[i]) == match.end())
            match.insert(std::pair<char, ll>(str2[i], 1));
        else
            match[str2[i]]++;
    }
    ll in = 0;
    for (int i = 0; i < str.length(); i++)
    {
        ...
        if (match.find(str[i]) == match.end() || match[str[i]] <= 0)
        {

            cout << "Case #" << caseOp << ": IMPOSSIBLE";
            caseOp++;
            return;
        }
        else
    ...
    cout << "Case #" << caseOp << ": " << (str2.length() - str.length());
    caseOp++;
}
int main()
{
