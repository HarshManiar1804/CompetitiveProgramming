...
int32_t main()
{
    ...
    int n, one = 0, zero = 0, zerom = 0, onem = 0, zeroc = 0, onec = 0;
    cin >> n;
    string s;
    cin >> s;
    fl(i, 0, n)
    {
        if (s[i] == '1')
            one++, onec++, onem = max(onec, onem), zeroc = 0;
        else
            zero++, zeroc++, zerom = max(zerom, zeroc), onec = 0;
    }
    ll ans = max(zerom * zerom, onem * onem);
