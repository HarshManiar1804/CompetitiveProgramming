...
void solve()
{
    ...
    ll leftCount = vec[0], rightCount = vec[n - 1], left = 0, right = n - 1;

    while (left < right)
    {
        if (leftCount < rightCount)
            left++, leftCount += vec[left];
        else if (rightCount < leftCount)
            right--, rightCount += vec[right];
        else
            count = left + 1 + n - right, left++, right--, leftCount += vec[left], rightCount += vec[right];
    }
    cout << count;
}
int main()
