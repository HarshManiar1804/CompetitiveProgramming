...
void ansGotai()
{
    int num, x, k, y, jawab = 0;
    string str;
    cin >> num;
    vll sodho(num);
    cin >> sodho;
    sort(sodho.begin(), sodho.end());
    fl(i,1,num)
        jawab = max(jawab, (abs(sodho[i] - sodho[i - 1]) + sodho[i] - sodho[0]));
    fl(i,0,num-1)
        jawab = max(jawab, abs(sodho[i + 1] - sodho[i]) + abs(sodho[i] - sodho[num - 1]));
    cout << jawab;
}
int32_t main()
{
