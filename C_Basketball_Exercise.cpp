...
void ansGotai()
{
    ...
    string str;
    cin >> num;
    vll vec(num), vec1(num);
    cin >> vec >> vec1;
    if (num == 1)
    {
        cout << max(vec[0], vec1[0]);
        return;
    }
    vll dp1(num + 1, 0), dp2(num + 1, 0);
    dp1[0] = 0, dp2[0] = 0;
    dp1[1] = vec[0], dp2[1] = vec1[0];

    for (auto i = 2; i <= num; i++)
    {
        dp1[i] = max(dp2[i - 1], dp2[i - 2]) + vec[i - 1];
        dp2[i] = max(dp1[i - 1], dp1[i - 2]) + vec1[i - 1];
    }
    cout << max(dp1[num], dp2[num]);
}
int32_t main()
{
